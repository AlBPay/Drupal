...
class FondyController extends ControllerBase {
	...
	public function complete( $cart_id = 0, Request $request ) {
		...
		if ( $valid == false ) {
			uc_order_comment_save( $order->id(), 0, $this->t( 'Attempted unverified Alliance bank completion for this order.' ), 'admin' );
			throw new AccessDeniedHttpException();
		}
		...
		if ( $request->request->get( 'order_status' ) == 'approved' && is_numeric( $request->request->get( 'amount' ) ) ) {
			$comment = $this->t( 'Paid by @type, example.com order #@order.', [
			...
			drupal_set_message( $this->t( 'Your order will be processed as soon as your payment clears at example.com.' ) );
			uc_order_comment_save( $order->id(), 0, $this->t( '@type payment is pending approval at example.com.', [ '@type' => $this->t( 'Credit card' ) ] ), 'admin' );
		}
	...
	public function notification( Request $request ) {
		...
		\Drupal::logger( 'uc_fondy' )->notice( 'Received Alliance bank notification with following data: @data', [ '@data' => print_r( $values->all(), true ) ] );
		...
		if ( $values->has( 'order_status' ) && $values->has( 'order_id' ) && $values->has( 'payment_id' ) ) {
			...
			if ( $valid == false ) {
				\Drupal::logger( 'uc_fondy' )->notice( 'Alliance bank notification #@num had a wrong Signs.', [ '@num' => $values->get( 'message_id' ) ] );
				die( 'Sign Incorrect' );
			}
			...
			switch ( $values->get( 'order_status' ) ) {
				case 'approved':
					if($order->getStateId() != 'payment_received') {
						$comment = $this->t( 'Alliance bank transaction ID: @payment_id', [ '@payment_id' => $values->get( 'order_id' ) ] );
						uc_payment_enter( $orderId, 'fondy', $values->get( 'amount' ) / 100, $order->getOwnerId(), null, $comment );
						uc_order_comment_save( $orderId, 0, $this->t( 'Alliance bank reported a payment of @amount @currency.', [
				...
				case 'declined':
					...
					uc_order_comment_save( $orderId, 0, $this->t( 'Order have not passed Alliance bank declined.' ) );
					die( 'canceled' );
					break;

				case 'expired':
					$order->setStatusId( 'canceled' )->save();
					uc_order_comment_save( $orderId, 0, $this->t( 'Order have not passed Alliance bank expired.' ) );
					die( 'canceled' );
					break;
			}
		}
